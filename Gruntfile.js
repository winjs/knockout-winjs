...
module.exports = function(grunt) {
  ...
  if (!fullWinjsVersion) {
    grunt.fail.fatal('Unable to determine WinJS version required by knockout-winjs');
  }
  ...
  grunt.initConfig({
    ...
    compress: {
      publish: {
        options: {
          archive: publishRoot + 'knockout-winjs.zip'
        },
        files: [{
          expand: true,
          cwd: npmPublishRoot,
    ...
    'github-release': {
      ...
      publish: {
        options: {
          repository: 'winjs/knockout-winjs',
          auth: {
          ...
          release: {
            ...
            name: '<%= pkg.version %>',
            body:
              'Release of knockout-winjs <%= pkg.version %>.\n' +
              '\n' +
              'Compatible with WinJS ' + winjsVersion + '.\n'
          }
        },
        files: {
          src: [publishRoot + 'knockout-winjs.zip']
        }
      }
    }
  });
  ...
  grunt.registerTask('finished-publish', function (mode) {
    grunt.log.writeln('');
    grunt.log.writeln('Publish complete. Hand tweak the GitHub release description if necessary (https://github.com/winjs/knockout-winjs/releases)');
    grunt.log.writeln('');
  });
  ...
  grunt.registerTask('publish', function (mode) {
    if (!mode) {
      grunt.log.writeln('');
      grunt.log.writeln('Will publish version ' + pkg.version + ' of knockout-winjs to npm, NuGet, etc. Double check that:');
